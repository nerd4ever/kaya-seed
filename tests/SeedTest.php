...
namespace Nerd4ever\Kaya\Seed\Test;
...
//        $this->assertTrue($this->tokenManager->revoke());
//        $this->assertFalse($this->tokenManager->refresh());
    }

    public function testWebhookCacheArtifact(): void
    {
        $wh = new WebhookManager($this->publishManager, 5);
        $this->assertInstanceOf(WebhookManagerInterface::class, $wh);
        $artifactOne = (new Artifact())->setId(to_uuidv4('kaya-seed-one'))
            ->setShortname('kaya-seed-one')
            ->setDisplayName('Simple Kaya Seed Example One')
            ->setEnabled(true);
        $artifactTwo = (new Artifact())->setId(to_uuidv4('kaya-seed-two'))
            ->setShortname('kaya-seed-two')
            ->setDisplayName('Simple Kaya Seed Example Two')
            ->setEnabled(true);

        $this->assertTrue($wh->add($artifactOne));
        $this->assertTrue($wh->add($artifactTwo));
        $this->assertFalse($wh->add($artifactOne));
    }
}